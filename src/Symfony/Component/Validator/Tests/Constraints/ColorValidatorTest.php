...
class ColorValidatorTest extends ConstraintValidatorTestCase
{
    ...
    {
        $this->validator->validate(new EmptyColorObject(), new Color());
     ...
     */
    public function testValidColors($email)
    {
    ...
    /**
     * @dataProvider getValidColorsWithWhitespaces
     */
    public function testValidNormalizedColors(string $color)
    {
        $this->validator->validate($color, new Color(['normalizer' => 'trim']));

        $this->assertNoViolation();
    }

    public function getValidColorsWithWhitespaces()
    {
        return [
            ["\x20#abc\x20"],
            ["\x09\x09rgb(0, 0, 0)\x09\x09"],
            ["\x0Argba(0,0,0,.5)\x0A"],
            ["\x0D\x0Dhsl(205,35%,17%)\x0D\x0D"],
            ["\x00#abc"],
            ["#abc\x0B\x0B"],
        ];
    }
